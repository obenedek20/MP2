...
class DecisionTree():
    ...
    def BestSplit(self, data: pd.DataFrame, label: pd.Series):
        ...
        for items in tot_vals:
            before_split_entropy -= items * np.log2(items)
        ...
        for col in data.columns: #should now handle non-boolean values
            ...
            if len(vals) > 2:
                ...
                for items in vals.index.sort_values(ascending=True):
                    ...
                    right_entr = 0
                    for p in left_freq:
                        left_entr -= p*np.log2(p)
                    for p in right_freq:
                        right_entr -= p*np.log2(p)
                    curr_entr = (len(left)/len(data_with_label))*(left_entr) + (len(right)/len(data_with_label))*(right_entr) #this could be wrong calculation
                    if curr_entr < best_entr:
                        best_entr = curr_entr
                        best_split = items
                ...
                for p in right_freq:
                    right_entr -= p*np.log2(p)
                best_entr = (len(left)/len(data_with_label))*(left_entr) + (len(right)/len(data_with_label))*(right_entr)
                
            entr = before_split_entropy - best_entr
            if split_feature == None:
                split_feature = col
                split_value = best_split
                split_ig = entr
            else:
                if tot_entropy < split_ig:
                    split_ig = entr
                    split_feature = col
                    split_value = best_split
